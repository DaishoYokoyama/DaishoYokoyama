...
札幌在住のフルスタックエンジニア。13 年のキャリアで、事業企画からリリースまでプロダクト開発の全工程に伴走してきました。信条は「速さ」と「優れた UX」の両立です。

直近は生成 AI / LLM の社会実装とスピード感のある PoC 開発に注力。RAG / LLM ワークフローの設計から、トークン課金やクラウドのランニングコストまで含めて ROI の合う形に落とし込むアーキテクチャ設計を得意としています。
